...
The generated stub in v1_thread.py returns a raw httpx Response;
use stream_thread_chat() here for a typed AsyncIterator instead.
"""
...
@dataclass
class ThreadStreamChunk:
    """A single SSE chunk from a workspace thread stream-chat response."""
    text_response: str
    id: str
    sources: List[ThreadStreamSource] = field(default_factory=list)
    close: bool = False
    error: Optional[str] = None

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ThreadStreamChunk":
        sources = [
            ThreadStreamSource(
                id=s.get("id", ""),
                url=s.get("url"),
...
async def stream_thread_chat(
...
) -> AsyncIterator[ThreadStreamChunk]:
    """
    ...
    Example::
        ...
        async for chunk in stream_thread_chat(sdk.v1._client, "my-workspace", "my-thread", {"message": "Hello"}):
            ...
            for line in lines:
                ...
                if line.startswith("data: "):
                    ...
                    try:
                        ...
                        continue
                    chunk = ThreadStreamChunk.from_dict(data)
                    if is_error_event:
